...
) -> Tuple[List[Action], dict, Dict[str, str]]:
            ...
            if act.kind == "mkdir":
                ...
                if not dry_run:
                    if act.path.exists() and not force:
                        print(
                            f"[WARN] Skipping existing file (use --force to overwrite): {act.path}"
                        )
                        continue
                ...
                if not dry_run:
                    if act.path.exists():
                        if force:
                            ...
                            print(
                                f"[WARN] Skipping existing symlink (use --force to overwrite): {act.path}"
                            )
                            continue
                    act.path.parent.mkdir(parents=True, exist_ok=True)
